...
func DeleteSharedVPCRole(sharedVPCRoleName string, managedPolicy bool, region string, awsSharedCredentialFile string) error {
	awsClient, err := aws_client.CreateAWSClient("", region, awsSharedCredentialFile)
	if err != nil {
		return err
	}
...
func DeleteSharedVPCChain(vpcID string, region string, awsSharedCredentialFile string) error {
	vpcClient, err := vpc_client.GenerateSharedVPCByID(vpcID, region, awsSharedCredentialFile)
	if err != nil {
		return err
	}
	return vpcClient.DeleteVPCChain(true)
